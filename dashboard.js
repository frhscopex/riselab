document.addEventListener("DOMContentLoaded", () => {
  ...
  function evaluatePasswordStrength(value) {
    ...
    if (score <= 2)
      return { className: "strength-weak", label: "Weak password" };
    if (score <= 4)
      return { className: "strength-medium", label: "Medium password" };
  ...
  document.querySelector(".avatar").textContent = displayName
    .split(" ")
    .map((n) => n[0])
    .join("")
    .slice(0, 2)
  ...
  async function fetchBilling() {
    try {
      ...
      if (data && data.data) {
        ...
        document.querySelector(".user-plan").textContent =
  ...
  window.closeDashboardAuthModals = function () {
    document
      .querySelectorAll("#dashboard-login-modal, #dashboard-signup-modal")
  ...
  function attachCopyHandlers() {
    document.querySelectorAll(".copy-key-btn").forEach((btn) => {
      ...
      btn.addEventListener("click", async () => {
        ...
        const keyToCopy = valueNode
          ? (valueNode.dataset.fullKey || valueNode.textContent || "").trim()
          : "";
    ...
    try {
      ...
      if (!items.length) {
        recentActivityFeed.innerHTML =
          '<li class="activity-empty">No memory logs yet.</li>';
        return;
      }

      recentActivityFeed.innerHTML = items
        .map(
          (item) => `
                <li>
      ...
      recentActivityFeed.innerHTML =
        '<li class="activity-empty">Activity feed is offline.</li>';
    }
  }
  ...
  function renderKnowledgeCards(items) {
    ...
    if (!items.length) {
      knowledgeResults.innerHTML =
        '<div class="knowledge-empty">No knowledge entries found.</div>';
      return;
    }

    knowledgeResults.innerHTML = items
      .map(
        (item) => `
  ...
  async function loadKnowledgeFeed() {
    ...
    try {
      ...
      knowledgeResults.innerHTML =
        '<div class="knowledge-empty">Knowledge feed is offline.</div>';
    }
  }
  ...
  async function searchKnowledge(query) {
    ...
    try {
      const res = await fetch(
        `${apiBaseUrl}/search?q=${encodeURIComponent(query)}&limit=8`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      knowledgeResults.innerHTML =
        '<div class="knowledge-empty">Search is currently unavailable.</div>';
    }
  }
  ...
  if (dashboardLoginForm) {
    ...
    dashboardLoginForm.addEventListener("submit", async (e) => {
      ...
      loginError.textContent =
        "Session-based login from dashboard will be enabled in the next update.";
      loginError.style.display = "block";
      setFormLoading(dashboardLoginForm, false);
    });
  }
  ...
  if (dashboardSignupForm) {
    ...
    const strengthContainer = document.getElementById(
      "dashboard-password-strength",
    );
    const strengthText = document.getElementById(
      "dashboard-password-strength-text",
    );
    ...
    dashboardSignupForm.addEventListener("submit", async (e) => {
      ...
      signupError.textContent =
        "Dashboard signup flow is a placeholder until social auth backend is connected.";
      signupError.style.display = "block";
      setFormLoading(dashboardSignupForm, false);
    });
